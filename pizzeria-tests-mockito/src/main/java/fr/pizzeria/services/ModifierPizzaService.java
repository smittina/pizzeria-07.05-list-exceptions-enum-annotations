...
public class ModifierPizzaService extends MenuService {
	...
	public void executeUC() throws PizzaException {
		...
		Pizza p = null;
		ok = false;
		// Récupération de l'ID en fonction du code Pizza
		Pizza pOriginal = getDao().findPizzaByCode(code);
		int idOriginal = pOriginal.getId();
		while(!ok) {
			System.out.println("Veuillez saisir la catégorie de la pizza : 1) VIANDE -- 2) POISSON -- 3) SANS_VIANDE");
			categorie = getScanner().next();
			switch(categorie) {
			case "VIANDE":
				p = new Pizza(idOriginal, newCode, newLibelle, newPrix, CategoriePizza.VIANDE);
				Validator.validatePizza(p);
				ok = true;
				break;
			case "POISSON":
				p = new Pizza(idOriginal, newCode, newLibelle, newPrix, CategoriePizza.POISSON);
				Validator.validatePizza(p);
				ok = true;
				break;
			case "SANS_VIANDE":
